...
vector<vector<int>> multiplyMatrix(const vector<vector<int>>& A, const vector<vector<int>>& B) {
    ...
    vector<vector<int>> P = multiplyMatrix(addMatrix(A11, A22), addMatrix(B11, B22));
    vector<vector<int>> Q = multiplyMatrix(addMatrix(A21, A22), B11);
    vector<vector<int>> R = multiplyMatrix(A11, subMatrix(B12, B22));
    vector<vector<int>> S = multiplyMatrix(A22, subMatrix(B21, B11));
    vector<vector<int>> T = multiplyMatrix(addMatrix(A11, A12), B22);
    vector<vector<int>> U = multiplyMatrix(subMatrix(A21, A11), addMatrix(B11, B12));
    vector<vector<int>> V = multiplyMatrix(subMatrix(A12, A22), addMatrix(B21, B22));

    vector<vector<int>> C11 = addMatrix(subMatrix(addMatrix(P, S), T), V);
    vector<vector<int>> C12 = addMatrix(R, T);
    vector<vector<int>> C21 = addMatrix(Q, S);
    vector<vector<int>> C22 = addMatrix(subMatrix(addMatrix(P, R), Q), U);
    for (int i = 0; i < half; i++) {
        for (int j = 0; j < half; j++) {
